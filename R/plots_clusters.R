...
#' `fviz_mca_biplot()`. Only the `"pca"` and `"mca"` embeddings are
#' supported, matching the biplot functions `factoextra` itself provides;
#' there is no `fviz_famd_biplot()` equivalent, and MDS has no variable
#' space to project.
#'
#' Three display variants, all following `factoextra`'s own documented
#' examples (`?factoextra::fviz_pca`):
#' \describe{
#'   \item{`"cluster"`}{Individuals colored and shaped by cluster assignment
#'     (`habillage`). The default.}
#'   \item{`"cos2"`}{Individuals colored by their quality of representation
#'     (`col.ind = "cos2"`) on a white-blue-orange gradient.}
#'   \item{`"label"`}{Individuals shown as text labels (observation names or
#'     row numbers) instead of points (`geom.ind = "text"`).}
#' }
#'
#' @param x A `cluster_fit`, `metacluster_fit`, or `cluster_explore` object.
#' @param data Optional numeric matrix, data frame, or distance object used to
...
#'   or `factoextra::fviz_mca_biplot()`, overriding the variant's defaults.
#'
#' @return A `ggplot` object.
#'
...
plot_biplot.cluster_explore <- function(x, data = NULL, embedding = c("auto", "pca", "mca"),
  ...
  variant_args <- switch(
    variant,
    cluster = list(habillage = x$plot_data$cluster, addEllipses = FALSE, label = "var", repel = TRUE),
    cos2 = list(
      col.ind = "cos2", geom.ind = "point",
      gradient.cols = c("white", "#2E9FDF", "#FC4E07"), repel = TRUE
    ),
    label = list(geom.ind = "text", repel = TRUE)
  )
  args <- utils::modifyList(variant_args, list(...))
  do.call(fviz_biplot, c(list(x$embedding_fit, select.var = select_var, title = "Cluster biplot"), args)) +
    theme_phynotype()
}
